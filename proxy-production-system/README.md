...
## 16) Logistics Tool (runbook 1-click)

Script `scripts/logistics_tool.py` gom các lệnh vận hành phổ biến thành một CLI duy nhất.

Xem trạng thái script, biến môi trường và lần chạy gần nhất:

```bash
python3 scripts/logistics_tool.py status
```

Chạy từng script con (forward args sau `--`):

```bash
python3 scripts/logistics_tool.py check-apis -- --output /tmp/platform-api-report.json
python3 scripts/logistics_tool.py monitor-orders -- --poll-seconds 30
python3 scripts/logistics_tool.py daily-report -- --force
```

Runbook quét đơn (clean -> build credentials -> scan):

```bash
python3 scripts/logistics_tool.py runbook-scan-pipeline --platform sapo
```

Runbook vận hành hàng ngày (check API -> build -> scan -> báo cáo Telegram):

```bash
set -a
source scripts/.env.vn-platforms.example
set +a
python3 scripts/logistics_tool.py --load-env runbook-daily-ops --authenticated
```

Dry-run để xem chuỗi lệnh trước khi chạy thật:

```bash
python3 scripts/logistics_tool.py runbook-daily-ops --dry-run --skip-report
```

Lịch sử chạy được lưu tại `/tmp/logistics-tool/run_history.json`.
